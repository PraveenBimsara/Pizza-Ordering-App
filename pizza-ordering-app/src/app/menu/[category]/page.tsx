...
const CategoryPage = () => {
  return (
    <div className='flex flex-wrap text-red-700'>
      {pizzas.map((item)=>(
        <Link className='w-full h-[60vh] border-r-2 border-b-2 border-red-700 sm:w-1/2 lg:w-1/3 p-4 flex flex-col justify-between group odd:bg-black' href={`/product/${item.id}`}>
          {/*IMAGE CONTAINER*/}
          {item.img && (
